<?php

namespace App\Http\Controllers\Main;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Models\default\TableMaster;
use Illuminate\Support\Facades\Validator;

class TableMasterController extends Controller
{
    public function create(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:50|unique:table_master,name',
        ], messages: [
            'name.required' => 'Description is required',
            'name.string' => 'Description must be a string',
            'name.max' => 'Description cannot exceed 50 characters',
            'name.unique' => 'This description is already taken',
        ]);


        if ($validator->fails()) {
            return response()->json(['error' => $validator->errors()], 422);
        }

        $user = TableMaster::create([
            'name' => $request->name,
            'capacity' => $request->capacity,
        ]);

        return response()->json(['message' => 'Table created successfully'], 201);
    }


    public function update(Request $request, $id)
    {
        // Validate the request data
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:50',

        ]);

        if ($validator->fails()) {
            return response()->json(['error' => $validator->errors()], 422);
        }

        // Find the ItemCategory by ID
        $itemCategory = TableMaster::find($id);

        // Check if the ItemCategory exists
        if (!$itemCategory) {
            return response()->json(['error' => 'Table not found'], 404);
        }

        // Update the ItemCategory
        $itemCategory->update([
            'name' => $request->name,
            'capacity' => $request->capacity,
            'is_sync' => 0,
            
        ]);

        return response()->json(['message' => 'Table updated successfully'], 200);
    }


    public function delete($id)
    {
        // Find the ItemCategory by ID
        $itemCategory = TableMaster::find($id);

        // Check if the ItemCategory exists
        if (!$itemCategory) {
            return response()->json(['error' => 'Table not found'], 404);
        }

        // Delete the ItemCategory
        $itemCategory->delete();

        return response()->json(['message' => 'Table deleted successfully'], 200);
    }

    public function index()
    {
        // Retrieve all ItemCategories
        $itemCategories = TableMaster::all();

        return response()->json($itemCategories);
    }

    public function show($id)
    {
        // Find the ItemCategory by ID
        $itemCategory = TableMaster::find($id);

        // Check if the ItemCategory exists
        if (!$itemCategory) {
            return response()->json(['error' => 'Table not found'], 404);
        }

        return response()->json(['Table' => $itemCategory], 200);
    }

}
