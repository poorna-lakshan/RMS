...
class SalesController extends Controller
{
    ...
    {
        return InvoiceHeader::selectRaw('
            invoice_header.customer_id,
            SUM(invoice_header.net_total) as total_sales,
            SUM(invoice_detail.qty) as total_qty
        ')
            ->join('invoice_detail', 'invoice_detail.reference_no', '=', 'invoice_header.id')
            ->with('customer')
            ->where('invoice_header.is_void', 0)
            ->where('invoice_header.customer_id', '<>', 1)
            ->groupBy('invoice_header.customer_id')
