...
{
    $customers = InvoiceHeader::selectRaw('
            customer_id,
...
public function allCustomersdetails()
{
    return InvoiceHeader::with(['customer', 'details.item'])
        ->where('is_void', 0)
        ->where('customer_id', '<>', 1)
        ...
        return response()->json([
            ...
            'getCreditInvoices' => $this->getCreditInvoices(),
             'allCustomers' => $this->allCustomers(),
              'allCustomersdetails' => $this->allCustomersdetails()
        ]);
    }
