        ...
        try {
            ...
            foreach ($items as $itemData) {
                ...
                if ($item) {
                    ...
                    if ($existingQty < 0) {
                        // Consume negative stock from this new batch
                        $negativeQty = abs($existingQty);

                        if ($batchQty >= $negativeQty) {
                            // Consume entire negative stock
                            $batchQty -= $negativeQty;
                            $batchData['qty'] = $batchQty;

                            // Update warehouse stock to zero after consuming negative
                            $newWarehouseQty = 0;
                        } else {
                            // Consume part of negative stock
                            $remainingNegative = $negativeQty - $batchQty;
                            $batchData['qty'] = 0;
                            $newWarehouseQty = -$remainingNegative;
                        }

                        // Create batch with adjusted qty
                        if ($batchData['qty'] > 0) {
