...
class SyncMasterDataController extends Controller
{
    ...
    {
        // Get the single company name
        $company = Company::first();

      $companyName = $company->name ?? '';
      $companyLogo = $company ? asset('storage/images/company/logo.png'): '';

        // Get all warehouses pending sync
        $warehouses = Warehouse::where('is_sync', 0)
            ->get()
            ->map(function ($wh) use ($companyName) {
                return [
                    'id' => $wh->id,
                    'name' => $companyName, // Always Company name
                    'code' => $companyName, // Always Company name
