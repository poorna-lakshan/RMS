...
class GRNController extends Controller
{
            ...
            foreach ($items as $itemData) {
                ...
                if ($item) {
                    ...
                    if ($existingQty < 0) {
                        ...
                        // Get negative batches (batches with negative qty)
                        $negativeBatches = ItemBatch::where('item_id', $item['id'])
                            ->where('ware_house_id', $request->warehouse_id)
                            ->where('qty', '<', 0)
                        ...
                        $remainingNegative = $negativeQty;
                        $batchQty = $newBaseQty; // GRN quantity

                        // First, consume negative batches with new stock
                        foreach ($negativeBatches as $negativeBatch) {
                            if ($batchQty <= 0 || $remainingNegative <= 0)
                                break;

                            $negativeBatchQty = abs($negativeBatch->qty); // Make positive for calculation

                            if ($batchQty >= $negativeBatchQty) {
                                // Enough new stock to fully consume this negative batch
                                $batchQty -= $negativeBatchQty;
                                $remainingNegative -= $negativeBatchQty;

                                // Update negative batch to zero (or mark as consumed)
                                $negativeBatch->qty = 0;
                                $negativeBatch->is_consumed = true;
                                $negativeBatch->notes = 'Negative stock consumed by GRN ' . $grn->code . ' on ' . now()->format('Y-m-d');
                                $negativeBatch->save();

                                \Log::info("Negative batch fully consumed", [
                                    'batch_id' => $negativeBatch->id,
                                    'batch_no' => $negativeBatch->batch_no,
                                    'consumed_qty' => $negativeBatchQty,
                                    'remaining_grn_qty' => $batchQty
                                ]);

                            } else {
                                // Only part of negative batch can be consumed
                                $negativeBatch->qty += $batchQty; // Add positive to negative (e.g., -5 + 3 = -2)
                                $remainingNegative -= $batchQty;
                                $batchQty = 0;

                                $negativeBatch->notes = 'Partially consumed by GRN ' . $grn->code . ' - Remaining: ' . $negativeBatch->qty;
                                $negativeBatch->save();
                        ...
                        // Now create batch with remaining GRN quantity (if any)
                        if ($batchQty > 0) {
                            $batchData['qty'] = $batchQty;
                            ItemBatch::create($batchData);

                        }

                        // Calculate new warehouse quantity
                        // Original negative + GRN quantity = new quantity
                        $newWarehouseQty = $existingQty + $newBaseQty;
                    ...
                    if (!$request->id) {
                        ...
                        ItemBatch::where('item_id', $itemData['id'])
                            ->where('ware_house_id', $request->warehouse_id)
                            ->where('qty', 0)
                            ->delete();
                    }
    ...
    {
        // Retrieve all GRNHeaders
        $itemCategories = GRNHeader::with(['vendor', 'warehouse', 'payments'])
            ->orderBy('id', 'desc')
            ->get();

        // Filter payments to only include non-void payments
        $itemCategories->each(function ($grn) {
            $grn->setRelation('payments', $grn->payments->where('is_void', false));
        });
