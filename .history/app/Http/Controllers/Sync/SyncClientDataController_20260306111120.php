...
class SyncClientDataController extends Controller
{
    ...
    public function sessionSync(Request $request)
    {
        ...
        if ($operation == 'START') {
            ...
            $inDate = date('Y-m-d', strtotime($data['SessionIn_Date']));
            $inTime = $data['SessionIn_Time'];

            CashierSessions::create([
                'code' => ('SESSION_' . $inDate . ' ' . $inTime),
                'in_time' => $inDate . ' ' . $inTime,
                'user' => $data['CurrentUser'],
                'is_in' => 1,
                'is_out' => 0,
                'in_amt' => $data['SessionIn_Amt'],
                'out_amt' => 0,
            ]);
        } elseif ($operation == 'END') {

            // UPDATE ONLY THE LAST OPEN SESSION
            $session = CashierSessions::where('user', $data['CurrentUser'])
                ->where('is_out', 0)
                ->latest('id')
                ->first();
            ...
            $outDate = date('Y-m-d', strtotime($data['SessionOut_Date']));
            $outTime = $data['SessionOut_Time'];

            $session->update([
                'out_time' => $outDate . ' ' . $outTime,
                'is_out' => 1,
                'out_amt' => $data['SessionOut_Amt'],
            ]);
        }
    ...
    public function getOrders()
    {
        ...
        $orders = OrderHeader::with(['details.item.kitchans', 'steward'])
            ...
            ->orWhere(function ($q) {
                $q->where('type', '<>', 'Table'); // Other types: take all, no invoice filter
            })
